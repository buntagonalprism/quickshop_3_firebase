...
The `firebase.json` file generated by the Firebase CLI when initialising firebase functions in a project reasonably includes a `predeploy` section that specifies to run a lint then build before deployment using `npm run lint` and `npm run build`. However when executing the deployment on a Github Actions CI server, execution of the predeploy steps resulted in the following error, [also mentioned on stackoverflow](https://stackoverflow.com/questions/77504522/firebase-deploy-error-running-command-npm-prefix-resource-dir-run-lint-r).
