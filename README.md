...
## Utility App 
The utility app is a Next.js application that handles sharing links for users that do not already have Quickshop installed. The app is built as a [static SPA](https://nextjs.org/docs/pages/building-your-application/deploying#static-html-export) with no server-side logic, and hosted in Firebase Hosting. See [Readme](./quickshop-utility-app/README.md) for details. 
...
1. Press **CTRL + SHIFT + B** to run the default build task "Start Firebase Emulators (Dev)" defined in `.vscode/tasks.json`. This task:
    1. Starts typescript compiler running using `npm run build:watch`. This will watch for changes to typescript files in `/functions/src` and compile them to javascript files output in `/functions/lib`
    2. Start firebase emulators using `firebase emulators:start --only "firestore,functions" --inspect-functions --project quickshop-dev`. The `--inspect-functions` argument exposes a debug port for VSCode to attach to
2. Press **F5** to start a debug session in Visual Studio Code using the "Debug Functions" launch configuration defined in `.vscode/launch.json`. This will will attach to the debug port exposed by the functions emulator, and breakpoints in the typescript files in `/functions/src` will be hit and the code can be stepped through and inspected. 
4. After editing any typescript files and saving:
    ...
    - The debug session in VSCode will be terminated and need to be restarted by pressing **F5** again. 
